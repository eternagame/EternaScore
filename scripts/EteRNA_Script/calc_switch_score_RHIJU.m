...
for j = which_sets
  ...
  for a = 1:2
    ...
    if ( size( all_area_pred{j}, 2 ) < 3 & a == 2 ) continue; end;
...
%    [d_on_norm, d_on_norm_err ] = get_data_norm( area_bsub{j}(:, (a-1) * 2 + 2), darea_bsub{j}(:, (a-1) * 2 + 2), goodbins);
  ...
  if ( j <= ceil(length(which_sets) / 2));
    plot_id = 2 * j - 1;
  else
    plot_id = j - ceil(length(which_sets) / 2);
    plot_id = 2 * plot_id;
  end
  subplot( ceil(length(which_sets)/2), 2, plot_id);
...
h = text( nres+1, 0.5, ['Sequence ',num2str(j),'\newline', strrep(design_names{j},'_','\_'), added_text],'fontsize',8,'fontwe','bold','verticalalign','top' );
  text( -0.5, 0.5, ['Switch\newlinescore: \newline',...
